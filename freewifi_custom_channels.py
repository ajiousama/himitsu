...
GCH_START='# === JRA_GCH_FREE_START ==='; GCH_END='# === JRA_GCH_FREE_END ==='; GUINEA_START='# === GUINEA_YOUTUBE_START ==='; GUINEA_END='# === GUINEA_YOUTUBE_END ==='; ECATV_START='# === EHIME_CATV_START ==='; ECATV_END='# === EHIME_CATV_END ==='
    ...
    while i<len(lines):
        ...
        if line.startswith('#EXTINF:') and i+1<len(lines):
            ...
            if any(base in url for base in (HARUKA_OLD_BASE,HARUKA_1_BASE,HARUKA_2_BASE)):
                ...
                if m:
                    path=m.group(1); name=re.sub(r'\s*\((?:haruka|ハルカ)[^)]*\)','',line,flags=re.I)
                    info1=re.sub(r',([^,]*)$',lambda x:','+x.group(1)+' (ハルカ1)',name); info2=re.sub(r',([^,]*)$',lambda x:','+x.group(1)+' (ハルカ2)',name)
                    out.extend([info1,HARUKA_1_BASE+path,info2,HARUKA_2_BASE+path]); i+=2; continue
        out.append(line); i+=1
    return '\n'.join(out).rstrip()+'\n'
...
def ensure_ecatv(text):
    ...
    a='# === GENERAL_YOUTUBE_MANAGED_START ==='; return text.replace(a,ECATV_BLOCK+'\n\n'+a,1) if a in text else text.rstrip()+'\n\n'+ECATV_BLOCK+'\n'

def patch_jra_youtube(text):
    lines=text.splitlines()
    for i,l in enumerate(lines):
        if l.startswith('#EXTINF:') and 'tvg-id="jra.official"' in l: lines[i]=f'#EXTINF:-1 tvg-id="jra.official" tvg-name="JRA公式（YouTube）無料版" tvg-logo="{JRA_YT_LOGO}" group-title="競馬",JRA公式（YouTube）無料版'
    return '\n'.join(lines).rstrip()+'\n'
...
def parse_xmltv_time(s):
    ...
    b=datetime.strptime(m.group(1),'%Y%m%d%H%M%S'); o=m.group(2)
    if o:
        sign=1 if o[0]=='+' else -1; return b.replace(tzinfo=timezone(sign*timedelta(hours=int(o[1:3]),minutes=int(o[3:5]))))
    return b.replace(tzinfo=JST)

def is_jra_race_day():
    try:
        if JRA_STATUS.exists():
            d=json.loads(JRA_STATUS.read_text(encoding='utf-8')); g=d.get('generated_at')
            if g and datetime.fromisoformat(g).astimezone(JST).date()==datetime.now(JST).date():return int(d.get('active_count') or 0)>0
    ...
    try:
        ...
        for p in root.findall('programme'):
            if (p.get('channel') or '') not in {'jra.east','jra.west','jra.hokkaido'}:continue
            s=parse_xmltv_time(p.get('start')); title=(p.findtext('title') or '').strip()
            if s and s.astimezone(JST).date()==today and title and not any(w in title for w in ('非開催','休止','準備中','データ取得準備中')):return True
        return False
...
def get_guinea_hls():
    try:
        ...
        u,_=direct_url(GUINEA_PAGE);return u
    except Exception as e:print('Guinea LIVE lookup failed:',e);return None

def main():
    text=FREEWIFI.read_text(encoding='utf-8-sig',errors='replace')
    text=suspend_5002(text); text=patch_haruka_sources(text); text=patch_jra_youtube(text); text=ensure_ecatv(text)
    if is_jra_race_day():
        ext=f'#EXTINF:-1 tvg-id="jra.gch.free" tvg-name="JRA公式（GCH）無料版【開催中のみ】" tvg-logo="{JRA_GCH_LOGO}" group-title="競馬",JRA公式（GCH）無料版【開催中のみ】'; block=f'{GCH_START}\n{ext}\n{GCH_URL}\n{GCH_END}'
    else:block=''
    text=replace_managed_block(text,GCH_START,GCH_END,block,'## 競馬\n'); gu=get_guinea_hls()
    block=f'{GUINEA_START}\n#EXTINF:-1 tvg-id="youtube.guinea" tvg-name="モルモット配信（YouTube）" tvg-logo="{GUINEA_LOGO}" group-title="動物",モルモット配信（YouTube）\n{gu}\n{GUINEA_END}' if gu else ''
    text=replace_managed_block(text,GUINEA_START,GUINEA_END,block,'# === GENERAL_YOUTUBE_MANAGED_END ===\n'); FREEWIFI.write_text(text.rstrip()+'\n',encoding='utf-8'); print('Custom FreeWiFi channels applied; 5002 suspended')
    try:
        from freewifi_today_public_sports import main as f; f()
    except Exception as e:print('Today public sports sync failed:',e)
    try:
        from apply_verified_status import main as f; f()
    except Exception as e:print('Verified status override failed:',e)
    try:
        from freewifi_keirin_repair import main as f; f()
