...
def title_matches(title, rule):
    return any(x in title for x in rule.get('include',[])) and not any(x in title for x in rule.get('exclude',[]))
    ...
    for m in PAIR_RE.finditer(text):
        ...
        for rule in rules:
            if title_matches(title,rule):
                found.append({'rule':rule,'title':title,'url':url,'start':start,'channel':attrs.get('channel','')})
                break
    # Newest first, remove duplicate programme/url pairs.
    seen=set(); out=[]
...
def build_m3u(items, group):
    lines=['#EXTM3U']
    kick_gccx=kick_has_gccx()
    for x in items:
        r=x['rule']
        # KICK is preferred for GCCX when KICK replay shelf already exists.
        if r.get('prefer_kick') and kick_gccx:
            continue
        dt=datetime.fromtimestamp(x['start'], timezone.utc).astimezone().strftime('%m/%d %H:%M')
        logo=RAW+r['logo']
        name=f"{r['name']} {dt}｜{x['title']}"
...
def main():
    ...
    epg=fetch_text(EPG_URL)
    candidates=parse_epg(epg,cfg['programmes'])
    if not candidates:
        raise SystemExit('HARU VOD: no favourite programmes found; refusing to overwrite shelf')
    checked=[x for x in candidates if probe(x['url'])]
    # If GitHub runner cannot probe HARU at all, do not erase a valid shelf.
    if PROBE and not checked:
        print('HARU VOD: all probes failed; using EPG candidates rather than publishing an empty shelf',file=sys.stderr)
        checked=candidates
    vod=build_m3u(checked,cfg.get('group_title','HARU VOD'))
