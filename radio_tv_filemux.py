...
# Normalize HLS timestamp jumps before MPEG-TS reaches television players.
# Keep the still picture as H.264 copy; only the inexpensive audio is encoded.
AUDIO_OUTPUT_ARGS = ["-af", "aresample=async=1000:first_pts=0",
...
def _file_cmd(station: str, source: str, path: pathlib.Path) -> list[str]:
    ...
    return [
        ...
        "-i", source,
        "-map", "0:v:0", "-map", "1:a:0",
        # Video is preencoded; AAC timestamps are repaired across HLS gaps.
        "-c:v", "copy",
        *AUDIO_OUTPUT_ARGS,
