...
def premium_login():
 ...
 return session
def auth(session):
 with open_url("https://radiko.jp/v2/api/auth1",headers=BASE_HEADERS,timeout=30) as r: token=r.headers["X-Radiko-AuthToken"]; off=int(r.headers["X-Radiko-KeyOffset"]); length=int(r.headers["X-Radiko-KeyLength"])
...
def _fetch_area_stations(n):
 ...
 for st in root.findall("station"):
  sid=(st.findtext("id") or "").strip(); name=(st.findtext("name") or sid).strip(); logo=""
  # Current radiko station XML commonly exposes logos as <logo width=... height=...>URL</logo>.
  # Keep legacy named tags as fallbacks.
  candidates=[]
  ...
  if not logo:
   logo=next(((st.findtext(tag) or "").strip() for tag in ("logo_large","logo_medium","logo_small","logo_xsmall") if (st.findtext(tag) or "").strip()),"")
...
class Handler(BaseHTTPRequestHandler):
 server_version="RadikoProxy/1.5"
 ...
 def do_GET(self):
  ...
  try:
   ...
   if p.path in ("/","/playlist.m3u"):
    ...
    self.send_bytes(200,("\n".join(lines)+"\n").encode(),"audio/x-mpegurl; charset=utf-8");return
   if p.path.startswith("/live/"):
    sid=urllib.parse.unquote(p.path.split("/",2)[2]);source,text=get_live_master(sid);self.send_bytes(200,rewrite_m3u(text,source,base_proxy).encode(),"application/vnd.apple.mpegurl");return
   if p.path=="/proxy":
    ...
    with fetch_proxied(target) as r:
     ...
     if is_m3u:data=rewrite_m3u(data.decode("utf-8","replace"),target,base_proxy).encode();ctype="application/vnd.apple.mpegurl"
     self.send_bytes(200,data,ctype or "application/octet-stream")
    return
   self.send_error(404)
  except Exception as e:self.send_error(502,str(e))
def main():
 print(f"radiko proxy listening: http://{HOST}:{PORT}",flush=True);print(f"VLC playlist: http://{HOST}:{PORT}/playlist.m3u",flush=True);print(f"XMLTV EPG: http://{HOST}:{PORT}/epg.xml",flush=True);ThreadingHTTPServer((HOST,PORT),Handler).serve_forever()
