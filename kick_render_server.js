...
const VERSION = "2026-09-24-render-v15-browser-grid";
...
async function fetchJson(url,opts={}){for(let a=0;a<3;a++){try{const r=await fetch(url,{...opts,headers:{accept:"application/json, text/plain, */*","user-agent":UA,...(opts.headers||{})},cache:"no-store"});if(r.ok){try{return await r.json()}catch{}}}catch{}if(a<2)await new Promise(r=>setTimeout(r,300*(a+1)))}return null}
...
async function alive(url){if(!url)return false;try{return (await getText(url)).includes("#EXTM3U")}catch{return false}}
...
function headers(res){res.setHeader("Cache-Control","no-store, max-age=0");res.setHeader("Pragma","no-cache");res.setHeader("Access-Control-Allow-Origin","*");res.setHeader("X-Media-Resolver-Version",VERSION)}
...
function m3u(res,text){headers(res);res.statusCode=200;res.setHeader("Content-Type","application/vnd.apple.mpegurl; charset=utf-8");res.end(text)}
  ...
  if(project&&ref){
    try{
      ...
      if(apiKey){
        ...
        if(findM3u8(play?.sources))throw new Error("TVer stream is DRM-protected");
      }
    }catch{}
  }
...
function allowedHaru(raw){try{const x=new URL(raw);return x.protocol==="https:"&&x.hostname==="haru.charandom.blog"&&/\/stream\/jp\/[^/]+\/replay\.m3u8$/i.test(x.pathname)}catch{return false}}
function pickVariant(text,source){const ls=String(text||"").replace(/\r/g,"").split("\n"),c=[];for(let i=0;i<ls.length;i++){if(!ls[i].trim().startsWith("#EXT-X-STREAM-INF:"))continue;const bw=Number((ls[i].match(/BANDWIDTH=(\d+)/i)||[])[1]||0);for(let j=i+1;j<ls.length;j++){const n=ls[j].trim();if(!n)continue;if(n.startsWith("#"))break;c.push({bw,url:abs(source,n)});break}}c.sort((a,b)=>b.bw-a.bw);return c[0]?.url||null}
function rewriteHaru(text,source,offset,duration){const ls=String(text||"").replace(/\r/g,"").split("\n"),header=[],groups=[];let tags=[],inf=null;const hdr=l=>l==="#EXTM3U"||/^#EXT-X-(VERSION|TARGETDURATION|MEDIA-SEQUENCE|DISCONTINUITY-SEQUENCE|PLAYLIST-TYPE|INDEPENDENT-SEGMENTS|ALLOW-CACHE)/.test(l);for(const raw of ls){let l=raw.trim();if(!l||l==="#EXT-X-ENDLIST")continue;if(l.startsWith("#EXTINF:")){inf={line:l,seconds:Number((l.match(/^#EXTINF:([0-9.]+)/)||[])[1]||0)};continue}if(l.startsWith("#")){l=l.replace(/URI="([^"]+)"/g,(_,x)=>`URI="${abs(source,x)}"`);if(!inf&&!groups.length&&!tags.length&&hdr(l))header.push(l);else tags.push(l);continue}if(inf){groups.push({tags,inf:inf.line,seconds:Math.max(0,inf.seconds||0),uri:abs(source,l)});tags=[];inf=null}}if(!groups.length)throw new Error("no media segments found");let cum=0,total=0,skipped=0,key=null,map=null;const sel=[];for(const g of groups){for(const t of g.tags){if(t.startsWith("#EXT-X-KEY:"))key=t;if(t.startsWith("#EXT-X-MAP:"))map=t}const s=cum,e=cum+g.seconds;cum=e;if(e<=offset+.001){skipped++;continue}if(s>=offset+duration-.001)break;const t=[...g.tags];if(!sel.length){if(key&&!t.some(x=>x.startsWith("#EXT-X-KEY:")))t.unshift(key);if(map&&!t.some(x=>x.startsWith("#EXT-X-MAP:")))t.unshift(map);t.unshift("#EXT-X-START:TIME-OFFSET=0,PRECISE=YES",`# HARU clip requested offset=${offset}s actual_segment_start=${s.toFixed(3)}s`)}sel.push({...g,tags:t});total+=g.seconds}if(!sel.length)throw new Error(`offset ${offset}s is outside available replay`);const out=header.map(l=>{const z=l.match(/^#EXT-X-MEDIA-SEQUENCE:(\d+)/);return z?`#EXT-X-MEDIA-SEQUENCE:${Number(z[1])+skipped}`:l});if(!out.includes("#EXTM3U"))out.unshift("#EXTM3U");for(const g of sel)out.push(...g.tags,g.inf,g.uri);out.push("#EXT-X-ENDLIST");return out.join("\n")+"\n"}
async function haruClip(u,res){const src=String(u.searchParams.get("src")||"").trim();let offset=Number(u.searchParams.get("offset")||0),duration=Number(u.searchParams.get("duration")||600);if(!Number.isFinite(offset))offset=0;if(!Number.isFinite(duration))duration=600;offset=Math.max(0,Math.min(Math.floor(offset),28800));duration=Math.max(30,Math.min(Math.floor(duration),3600));if(!allowedHaru(src))return json(res,400,{error:"invalid HARU replay source",resolver:VERSION});try{let media=src,text=await getText(media,"https://haru.charandom.blog/");for(let d=0;d<2&&/#EXT-X-STREAM-INF:/i.test(text);d++){const v=pickVariant(text,media);if(!v)throw new Error("master playlist has no variant");media=v;text=await getText(media,"https://haru.charandom.blog/")}if(/#EXT-X-STREAM-INF:/i.test(text))throw new Error("nested master playlist too deep");return m3u(res,rewriteHaru(text,media,offset,duration))}catch(e){return json(res,502,{error:"HARU clip playlist failed",offset,duration,detail:String(e?.message||e),resolver:VERSION})}}


function execFileBuffer(file,args,timeout=30000){return new Promise((resolve,reject)=>{execFile(file,args,{encoding:null,maxBuffer:8*1024*1024,timeout},(err,stdout,stderr)=>{if(err)return reject(new Error((stderr||Buffer.alloc(0)).toString("utf8").slice(-1200)||err.message));resolve(stdout)})})}
async function kickFrame(u,res){
  ...
  let t=Number(u.searchParams.get("time")||0);
  if(!Number.isFinite(t)||t<0)t=0;
  t=Math.floor(Math.min(t,604800));
...
function pickLowestVariant(text,source){const ls=String(text||"").replace(/\r/g,"").split("\n"),c=[];for(let i=0;i<ls.length;i++){if(!ls[i].trim().startsWith("#EXT-X-STREAM-INF:"))continue;const bw=Number((ls[i].match(/BANDWIDTH=(\d+)/i)||[])[1]||0);for(let j=i+1;j<ls.length;j++){const n=ls[j].trim();if(!n)continue;if(n.startsWith("#"))break;c.push({bw,url:abs(source,n)});break}}c.sort((a,b)=>a.bw-b.bw);return c[0]?.url||null}
async function kickBoundary(u,res){
  const vod=String(u.searchParams.get("vod")||"").trim();
  if(!/^[A-Za-z0-9_-]{6,120}$/.test(vod))return json(res,400,{error:"invalid KICK VOD id",resolver:VERSION});
  let center=Number(u.searchParams.get("center")||0),span=Number(u.searchParams.get("span")||600);
  if(!Number.isFinite(center)||center<0)center=0;if(!Number.isFinite(span)||span<60)span=600;span=Math.min(Math.floor(span),900);
  const data=await getJson("https://kick.com/api/v1/video/"+encodeURIComponent(vod));
  if(!data)return json(res,404,{error:"KICK VOD metadata unavailable",vod,resolver:VERSION});
...
function kickBrowserGrid(u,res){
  const vod=String(u.searchParams.get("vod")||"").trim();
  if(!/^[A-Za-z0-9_-]{6,120}$/.test(vod))return json(res,400,{error:"invalid KICK VOD id",resolver:VERSION});
  let start=Number(u.searchParams.get("start")||0),step=Number(u.searchParams.get("step")||300),count=Number(u.searchParams.get("count")||12);
  if(!Number.isFinite(start)||start<0)start=0;if(!Number.isFinite(step)||step<30)step=300;if(!Number.isFinite(count)||count<1)count=12;
  start=Math.floor(start);step=Math.floor(step);count=Math.min(18,Math.floor(count));
  headers(res);res.statusCode=200;res.setHeader("Content-Type","text/html; charset=utf-8");
  res.end(`<!doctype html><meta charset="utf-8"><title>GCCX browser cue grid</title>
<style>body{margin:12px;background:#111;color:#fff;font:15px sans-serif}.grid{display:grid;grid-template-columns:repeat(3,1fr);gap:10px}.card{background:#222;padding:6px}.card canvas{width:100%;height:auto;background:#000}.lab{font-weight:700;padding:4px}.status{position:sticky;top:0;background:#400;padding:8px;z-index:9}</style>
<div id="status" class="status">loading HLS…</div><video id="v" muted playsinline crossorigin="anonymous" style="position:absolute;width:1px;height:1px;opacity:.01"></video><div id="grid" class="grid"></div>
<script src="https://cdn.jsdelivr.net/npm/hls.js@1.6.13/dist/hls.min.js"></script>
<script>
const vod=${JSON.stringify(vod)}, start=${start}, step=${step}, count=${count};
const times=Array.from({length:count},(_,i)=>start+i*step), v=document.getElementById('v'), grid=document.getElementById('grid'), st=document.getElementById('status');
function label(t){const h=Math.floor(t/3600),m=Math.floor((t%3600)/60),s=t%60;return (h?String(h).padStart(2,'0')+':':'')+String(m).padStart(2,'0')+':'+String(s).padStart(2,'0')}
const cards=times.map(t=>{const d=document.createElement('div');d.className='card';const c=document.createElement('canvas');c.width=640;c.height=360;const l=document.createElement('div');l.className='lab';l.textContent=t+'s — '+label(t);d.append(c,l);grid.append(d);return {t,c,l}});
function once(el,ev,ms=15000){return new Promise((ok,no)=>{let z=setTimeout(()=>{el.removeEventListener(ev,f);no(new Error(ev+' timeout'))},ms);function f(){clearTimeout(z);el.removeEventListener(ev,f);ok()}el.addEventListener(ev,f,{once:true})})}
async function capture(card){st.textContent='seeking '+card.t+'s ('+label(card.t)+')';v.currentTime=card.t;await once(v,'seeked',20000);await new Promise(r=>setTimeout(r,450));const x=card.c.getContext('2d');x.drawImage(v,0,0,640,360);x.fillStyle='rgba(0,0,0,.72)';x.fillRect(0,320,640,40);x.fillStyle='#fff';x.font='bold 24px sans-serif';x.fillText(card.t+'s  '+label(card.t),12,348);card.l.textContent=card.t+'s — '+label(card.t)+' ✓'}
async function run(){try{for(const c of cards)await capture(c);st.textContent='READY — '+count+' frames captured';st.style.background='#064'}catch(e){st.textContent='ERROR: '+e;st.style.background='#900'}}
if(window.Hls&&Hls.isSupported()){const h=new Hls({startPosition:0,maxBufferLength:12,backBufferLength:0,maxMaxBufferLength:20});h.loadSource('/kick?vod='+encodeURIComponent(vod));h.attachMedia(v);h.on(Hls.Events.MANIFEST_PARSED,()=>{v.pause();run()});h.on(Hls.Events.ERROR,(e,d)=>{if(d.fatal){st.textContent='HLS ERROR '+d.type+' '+d.details;st.style.background='#900'}})}else if(v.canPlayType('application/vnd.apple.mpegurl')){v.src='/kick?vod='+encodeURIComponent(vod);v.addEventListener('loadedmetadata',run,{once:true})}else{st.textContent='HLS unsupported';st.style.background='#900'}
</script>`);
}

async function handler(req,res){const u=new URL(req.url,"http://localhost");if(u.pathname==="/health")return json(res,200,{ok:true,resolver:VERSION,features:["kick-live","kick-vod","kick-vod-clip","kick-frame","kick-grid","kick-browser-grid","kick-boundary","haru-clip","tver-vod"]});if(u.pathname==="/kick-browser-grid"||u.pathname==="/api/kick-browser-grid")return kickBrowserGrid(u,res);if(u.pathname==="/kick-grid"||u.pathname==="/api/kick-grid")return kickGrid(u,res);if(u.pathname==="/kick-boundary"||u.pathname==="/api/kick-boundary")return kickBoundary(u,res);if(u.pathname==="/kick-frame"||u.pathname==="/api/kick-frame")return kickFrame(u,res);if(u.pathname==="/tver"||u.pathname==="/api/tver")return tverVod(u,res);if(u.pathname==="/haru-clip"||u.pathname==="/api/haru-clip")return haruClip(u,res);if(u.pathname!=="/kick"&&u.pathname!=="/api/kick"&&u.pathname!=="/api/kick-replay")return json(res,404,{error:"not found",resolver:VERSION});if(u.searchParams.has("vod"))return kickVod(u,res);const aliases={gccx:"kick.gccx",gccx2:"kick.gccx2",nogizaka:"kick.nogizaka",nogi:"kick.nogizaka",kujotaizai:"kick.kujotaizai",kujo:"kick.kujotaizai",wekiukk7:"kick.kujotaizai"};const key=String(u.searchParams.get("ch")||"").toLowerCase(),id=aliases[key]||key,item=channels.find(x=>String(x.tvg_id||"").toLowerCase()===id);if(!item)return json(res,404,{error:"unknown KICK channel",resolver:VERSION});const hit=await resolveLive(item);if(!hit)return json(res,503,{error:"KICK live playback unavailable",tvg_id:item.tvg_id,slug:item.slug,resolver:VERSION});return redirect(res,hit.playback)}
