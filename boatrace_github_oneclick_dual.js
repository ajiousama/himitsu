...
async function sendToTarget(target, boat) {
  ...
  const pushed = await upsertGitHubFile(target, boat.output, token, `Update BOAT RACE M3U ${boat.date} from iPhone one-click`);
...
try {
  ...
  const a = new Alert();
  a.title = errors.length ? "⚠️ 公営これ一発 一部エラー" : "🚤 公営これ一発 完了";
  a.message = msg;
  a.addAction("OK");
  await a.present();
} catch (e) {
