...
PROXY = "https://freewifi-media.onrender.com/kick?ch=gccx2"
...
def main():
    ...
    if data is None:
        ...
        if old_url:
            ...
            if new_text != text:
                ...
                print("KICK lookup failed; preserved CX2 via stable Render proxy")
            else:
    ...
    if new_text != text:
        FREEWIFI.write_text(new_text, encoding="utf-8")
        if live:
            print("CX2 Free Wi-Fi state changed: LIVE -> stable Render KICK proxy")
        else:
