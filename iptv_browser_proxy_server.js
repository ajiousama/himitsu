...
const ALLOWED = ['streaks.jp','googlevideo.com','youtube.com','youtu.be','youtube-nocookie.com','ytimg.com','charandom.blog','jp-primehome.com','boatrace.jp','jlc.ne.jp','githubusercontent.com','github.com','greenchannel.jp'];
...
function allowedHost(h){h=h.toLowerCase().replace(/\.$/,'');return EXACT.has(h)||ALLOWED.some(s=>h===s||h.endsWith('.'+s));}
async function checked(raw){let u;try{u=new URL(raw);}catch{throw Error('invalid url');}if(!['http:','https:'].includes(u.protocol)||u.username||u.password)throw Error('unsupported url');const h=u.hostname.toLowerCase();if(!allowedHost(h))throw Error('target host not allowed');if(net.isIP(h)){if(privateIp(h))throw Error('private target blocked');return u;}const a=await dns.lookup(h,{all:true,verbatim:true});if(!a.length||a.some(x=>privateIp(x.address)))throw Error('private target blocked');return u;}
function upHeaders(u,req){const h=new Headers({'User-Agent':UA,'Accept':req.headers.accept||'*/*','Accept-Language':req.headers['accept-language']||'ja-JP,ja;q=.9,en;q=.7','Cache-Control':'no-cache','Pragma':'no-cache'});if(req.headers.range)h.set('Range',req.headers.range);const host=u.hostname.toLowerCase(),full=u.href.toLowerCase();if(host.endsWith('streaks.jp')){if(full.includes('boatrace')||full.includes('cp-boatrace-prod')){h.set('Origin','https://front.player.boatrace-cdn.jp');h.set('Referer','https://front.player.boatrace-cdn.jp/');}else{h.set('Origin','https://tver.jp');h.set('Referer','https://tver.jp/');}}else if(host.endsWith('googlevideo.com')){h.set('Origin','https://www.youtube.com');h.set('Referer','https://www.youtube.com/');}else if(host.endsWith('charandom.blog'))h.set('Referer','https://haru.charandom.blog/');else if(host==='118.21.101.221'){const port=u.port||'80';const cam=port==='81'?'cam02':port==='90'?'cam03':'cam01';h.set('Referer',`https://www.kankoko.com/cam/web-cam/${cam}/index.html`);h.set('Accept','multipart/x-mixed-replace,image/jpeg,image/*,*/*;q=.8');}return h;}
async function getUp(raw,req){let u=await checked(raw);for(let i=0;i<6;i++){const ac=new AbortController(),t=setTimeout(()=>ac.abort(),20000);let r;try{r=await fetch(u,{method:req.method==='HEAD'?'HEAD':'GET',headers:upHeaders(u,req),redirect:'manual',signal:ac.signal});}finally{clearTimeout(t);}if(r.status>=300&&r.status<400&&r.headers.get('location')){u=await checked(new URL(r.headers.get('location'),u).href);continue;}return r;}throw Error('too many redirects');}
...
function extractBalancedObject(text,marker){
  ...
  const start=text.indexOf('{',p+marker.length);if(start<0)return null;
...
function findFirstVideoId(node){
  ...
  if(Array.isArray(node)){
    for(const x of node){const id=findFirstVideoId(x);if(id)return id;}
    return null;
  }
  if(typeof node!=='object')return null;
  for(const key of ['videoRenderer','gridVideoRenderer']){
    const r=node[key];
    if(r&&/^[A-Za-z0-9_-]{11}$/.test(r.videoId||''))return r.videoId;
  }
  for(const v of Object.values(node)){const id=findFirstVideoId(v);if(id)return id;}
  return null;
}
async function searchYouTubeLive(q){
  const url='https://www.youtube.com/results?search_query='+encodeURIComponent(q)+'&sp=EgJAAQ%253D%253D&hl=ja';
  const ac=new AbortController(),t=setTimeout(()=>ac.abort(),20000);
  let r;
  try{
    r=await fetch(url,{headers:{'User-Agent':UA,'Accept-Language':'ja-JP,ja;q=.9,en;q=.7','Cache-Control':'no-cache'},redirect:'follow',signal:ac.signal});
  }finally{clearTimeout(t);}
  if(!r.ok)throw Error('youtube search '+r.status);
  const html=await r.text();
  let data=extractBalancedObject(html,'var ytInitialData = ');
  if(!data)data=extractBalancedObject(html,'ytInitialData = ');
  let id=findFirstVideoId(data);
  if(!id){
    const m=html.match(/"videoId":"([A-Za-z0-9_-]{11})"/);
    if(m)id=m[1];
  }
  if(!id)throw Error('live video not found');
  return id;
}
...
const server=http.createServer(async(req,res)=>{cors(res);const parsed=new URL(req.url,'http://'+(req.headers.host||'localhost'));if(req.method==='OPTIONS'){res.statusCode=204;return res.end();}
  if(parsed.pathname==='/'||parsed.pathname==='/health'){res.setHeader('Content-Type','application/json; charset=utf-8');res.setHeader('Cache-Control','no-store');return res.end(JSON.stringify({ok:true,service:'iptv-9x-browser-proxy',version:'3'}));}

  if(parsed.pathname==='/yt-live'){
    if(!['GET','HEAD'].includes(req.method)){res.statusCode=405;return res.end('method not allowed');}
    const q=(parsed.searchParams.get('q')||'').trim();
    if(!q||q.length>180){res.statusCode=400;return res.end('bad query');}
    try{
      const id=await searchYouTubeLive(q);
      ...
      res.setHeader('X-YouTube-Video-Id',id);
      ...
      res.setHeader('Cache-Control','no-store');
      return res.end('youtube live resolver error: '+(e.message||e));
    }
  }
...
server.listen(PORT,'0.0.0.0',()=>console.log('IPTV browser proxy v2 listening on',PORT));
