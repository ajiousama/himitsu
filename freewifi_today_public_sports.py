...
def epg_state():
    ...
    for p in root.findall('programme'):
        ...
        if not start or start.date() != today: continue
        title = (p.findtext('title') or '').strip()
        desc = (p.findtext('desc') or '').strip()
        compact = ''.join(title.split())
        if not compact or any(x in compact for x in NON_EVENT_WORDS): continue
        real.add(cid)
        joined = title + ' ' + desc
        ...
        tm = re.search(r'([0-2]?\d:[0-5]\d)\s*発走', title)
        if m and tm:
            trans = str.maketrans('０１２３４５６７８９','0123456789')
            hh, mm = map(int, tm.group(1).split(':'))
            dt = datetime.combine(today, datetime.min.time(), tzinfo=JST).replace(hour=hh, minute=mm)
            if dt >= now:
...
def replace_block(text, payload):
    ...
    if pat.search(text): return pat.sub(payload+'\n', text, count=1)
...
def main():
    ...
    for cid, (section, block) in entries.items():
        ...
        b = block[:]; b[0] = sanitize_extinf(b[0]); name = entry_name(b); nr = next_race.get(cid)
        rows.append({'id':cid,'name':name,'block':b,'next_race':nr})
